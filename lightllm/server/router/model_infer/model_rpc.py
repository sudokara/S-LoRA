...
class ModelRpcServer(rpyc.Service):
    ...
    def exposed_init_model(self, kvargs):
        ...
        model_kvargs = {
            ...
            "lora_num": kvargs.get("lora_num", 1), 
            "lora_rank": kvargs.get("lora_rank", 64),
            "max_prefill_batch_size": kvargs.get("max_prefill_batch_size", 2),
            "token_per_lora": kvargs.get("token_per_lora", 16)
        }
    ...
    def exposed_prefill_batch(self, batch_id, aaas_mode=None):
        return self.forward(batch_id, is_prefill=True, aaas_mode=aaas_mode)

    @calculate_time(show=True, min_cost_ms=200)
    def exposed_decode_batch(self, batch_id, aaas_mode=None):
        return self.forward(batch_id, is_prefill=False, aaas_mode=aaas_mode)
    ...
    def exposed_load_lora_prefill(self, i):
        return self.model.gpu_lora.load_lora_prefill(i)

    def exposed_prepare_lora_prefill(self, i):
        return self.model.gpu_lora.prepare_lora_prefill(i)

    def exposed_swap(self, i):
        return self.model.gpu_lora.swap(i)
...
class ModelRpcClient:
    def __init__(self, model_rpc, world_size, rpc_server_process=None):
        ...
        if self.use_rpc:
            ...
            self._swap = async_wrap(self.model.swap)
            self._load_lora_prefill = async_wrap(self.model.load_lora_prefill)
            self._prepare_lora_prefill = async_wrap(self.model.prepare_lora_prefill)
            self._merge_lora_prefill = async_wrap(self.model.merge_lora_prefill)
        else:
            ...
            self._swap = self.model.exposed_swap
            self._load_lora_prefill = self.model.exposed_load_lora_prefill
            self._prepare_lora_prefill = self.model.exposed_prepare_lora_prefill
            self._merge_lora_prefill = self.model.exposed_merge_lora_prefill
        return
    ...
    async def prefill_batch(self, batch_id, aaas_mode:str=None):
        ans = self._prefill_batch(batch_id, aaas_mode=aaas_mode)
        if self.use_rpc:
            return await ans
        else:
            return ans

    async def decode_batch(self, batch_id, aaas_mode:str=None):
        ans = self._decode_batch(batch_id, aaas_mode=aaas_mode)
        if self.use_rpc:
            return await ans
        else:
            return ans
    ...
    async def load_lora_prefill(self, i):
        ans = self._load_lora_prefill(i)
        if self.use_rpc:
            await ans
            return
        else:
    ...
    async def prepare_lora_prefill(self, i):
        ans = self._prepare_lora_prefill(i)
        if self.use_rpc:
            await ans
            return
        else:
    ...
    async def merge_lora_prefill(self, i):
        ans = self._merge_lora_prefill(i)
        if self.use_rpc:
            await ans
            return
        else:
